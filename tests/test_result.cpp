...
TEST(ResultTest, NonSuccessTaskResultsDoNotMaterializePayload) {
    ...
    const auto abort_chain = yorch::task_result<int>::abort_chain();
    const auto abort_schedule = yorch::task_result<int>::abort_schedule();
    ...
    EXPECT_EQ(abort_chain.step.status, yorch::step_status::abort_chain);
    EXPECT_EQ(abort_schedule.step.status, yorch::step_status::abort_schedule);
    EXPECT_EQ(from_step.step.status, yorch::step_status::failure);
    EXPECT_FALSE(failure.has_value());
    EXPECT_FALSE(skip.has_value());
    EXPECT_FALSE(retry.has_value());
    EXPECT_FALSE(abort_chain.has_value());
    EXPECT_FALSE(abort_schedule.has_value());
