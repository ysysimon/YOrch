...
void run_plan_runtime_case(benchmark::State& state) {
    int sink = 0;
...
[[nodiscard]] std::string make_case_name(
    ...
    return std::string {"RunPlan/Runtime/"} + topology + "/" + payload + "/" +
...
void register_case(const char* slot_layout, const char* exec_policy) {
    benchmark::RegisterBenchmark(
        make_case_name(
            ...
            slot_layout,
            exec_policy),
        &run_plan_runtime_case<Scenario, LayoutPolicy, ExecPolicy>);
}
...
void register_run_plan_runtime_cases() {
...
const bool registered = [] {
    register_run_plan_runtime_cases();
    return true;
}();
